...
async def lifespan(app: FastAPI):
    """Manages the initialization and termination of the Matter hardware subprocess."""
...
app = FastAPI(title="Matter API Bridge", lifespan=lifespan)
...
async def serve_all_devices_api():
    """Retrieves identical cache representation of the current local device states."""
...
async def serve_set_api(request: Request, payload: Optional[ControlPayload] = None):
    """Dispatches physical actuation requests parsed mathematically against Matter clusters."""
    verify_hardware_readiness()
    ...
    try:
        ...
        return {"status": "success", "id": resolved_id}
    except Exception as e:
        logging.error(f"Command execution failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.api_route("/api/callback", methods=["GET", "POST"])
async def serve_callback_api(request: Request, payload: Optional[CallbackPayload] = None):
    """Maps deterministic execution binaries executed directly on host filesystem."""
    if request.method == "POST" and payload:
        device_id = payload.id
    ...
    if not device_id or not script_path:
        raise HTTPException(status_code=400, detail="Missing id or script parameter")
        
    resolved_id = bridge_instance.resolve_id(device_id)
    
    if not os.path.isfile(script_path):
        raise HTTPException(status_code=400, detail="Specified script file does not exist")
        
    bridge_instance.occupancy_callbacks[resolved_id] = script_path
    bridge_instance._save_callbacks()
    
    return {"status": "success", "id": resolved_id, "script": script_path}
